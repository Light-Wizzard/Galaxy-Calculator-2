...
for file in "$@"; do
    if [ ! -e "$file" ]; then echo "$file is missing, giving up." >&2; exit 1; fi
done

if [ $# -eq 0 ]; then echo "No artifacts to use for release, giving up."; exit 0; fi
...
if [ "$ARTIFACTORY_BASE_URL" != "" ]; then
  ...
  if [ "$ARTIFACTORY_API_KEY" == "" ]; then echo "Please set ARTIFACTORY_API_KEY"; exit 1; fi
  # files="$@";
...
if [ "$TRAVIS_EVENT_TYPE" == "pull_request" ] ; then
  ...
  if [ "$ARTIFACTORY_BASE_URL" != "" ]; then
    ...
    for FILE in "$@" ; do
      ...
      # cat ./one-upload; # this way we get a newline
      print -n "$(cat ./one-upload)\\n" >> ./uploaded-to; # this way we get a \n but no newline
    done
  fi
...
if [ "$TRAVIS_COMMIT" != "$target_commit_sha" ] ; then
  ...
  # echo "release_url: $release_url"
  # curl -XGET --header "Authorization: token ${GITHUB_TOKEN}" "$release_url"
  ...
  release_infos="$(curl -H "Authorization: token ${GITHUB_TOKEN}" --data '{"tag_name": "'"$RELEASE_NAME"'","target_commitish": "'"$TRAVIS_COMMIT"'","name": "'"$RELEASE_TITLE"'","body": "'"$BODY"'","draft": false,"prerelease": '$is_prerelease'}' "https://api.github.com/repos/$REPO_SLUG/releases")";
...
if [ -z "$release_url" ] ; then	echo "Cannot figure out the release URL for $RELEASE_NAME";	exit 1; fi
...
for FILE in "$@" ; do
  FULLNAME="${FILE}";
  BASENAME="$(basename "${FILE}")";
  curl -H "Authorization: token ${GITHUB_TOKEN}" -H "Accept: application/vnd.github.manifold-preview" -H "Content-Type: application/octet-stream" --data-binary @$FULLNAME "$upload_url?name=$BASENAME";
  echo "";
done
